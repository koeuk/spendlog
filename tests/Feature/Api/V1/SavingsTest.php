...
class SavingsTest extends TestCase
{
    ...
    public function test_money_taken_back_out_in_the_same_month_still_counts_as_saved(): void
    {
        $user = User::factory()->create();
        ...
        SavingsEntry::factory()->for($user)->withdrawal(50)->create(['saved_on' => '2026-09-22']);

        Sanctum::actingAs($user, [TokenAbility::SavingsRead->value]);

        $this->getJson('/api/v1/savings/summary?month=2026-09')
            ->assertOk()
            // 100 went aside this month. The 50 that came back out is a
            // movement of the balance, not a smaller month.
            ->assertJsonPath('data.saved_this_month', '100.00')
            ->assertJsonPath('data.remaining', '50.00')
            ->assertJsonPath('data.percent', 67)
            ->assertJsonPath('data.percent_raw', 67)
            // The balance is where the withdrawal is read.
