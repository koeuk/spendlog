...
SpendLog is one Laravel API with four faces: an Inertia/Vue web app, a Flutter
app, an Expo/React Native app, and the JSON API itself. Unless a line says
otherwise, a feature is on all of them.
...
- Email verification is enforced on the web; the mobile clients do not
  surface it
- The Help page and CMS pages are on web and native, not in the Flutter app
